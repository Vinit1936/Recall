...
export default function DailyRevisionPage() {
  ...
  return (
    <div>
      ...
      <StatsStrip streak={streak} totalProblems={totalProblems} masteredCount={masteredCount} dueCount={dueCount} />

      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 600, color: '#fff', margin: 0 }}>⌈ Daily Revision ⌋</h1>
        <span style={{ fontFamily: 'var(--font-geist-mono), monospace', fontSize: 14, color: '#666' }}>{dateLabel}</span>
      </div>
      ...
      <AnimatePresence mode="wait">
        {dueLoading ? (
          ...
          <motion.div key="list" initial={{ opacity: 1 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }}>
            {overdue.length > 0 && (
              <div style={{ marginBottom: 24 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                  <span style={{ fontSize: 14, color: '#f87171' }}>⚠</span>
                  <span style={{ fontFamily: 'var(--font-geist-mono), monospace', fontSize: 12, color: '#f87171', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Overdue ({overdue.length})</span>
                </div>
                {overdue.map((p: any) => <ProblemRevisionRow key={p.id} problem={p} onRevised={handleRevised} onToast={showToast} />)}
              </div>
            )}
            {dueToday.length > 0 && (
              <div>
                <div style={{ marginBottom: 8 }}>
                  <span style={{ fontFamily: 'var(--font-geist-mono), monospace', fontSize: 12, color: '#888', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Today ({dueToday.length})</span>
                </div>
                {dueToday.map((p: any) => <ProblemRevisionRow key={p.id} problem={p} onRevised={handleRevised} onToast={showToast} />)}
              </div>
            )}
          </motion.div>
        )}
